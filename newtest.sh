...
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
echo "Blogger博客 ：ygkkk.blogspot.com"
...
if [ -z "$latest_version" ]; then
    echo "获取版本信息失败，使用备用方法..."
    latest_version="v1.8.24"  # 备用版本
fi
echo "下载Xray官方版本：$latest_version"
...
# 下载并解压Xray
curl -Lo "$HOME/agsb/xray.zip" -# --retry 2 "https://github.com/XTLS/Xray-core/releases/download/$latest_version/$xray_file"
if [ $? -eq 0 ] && [ -f "$HOME/agsb/xray.zip" ]; then
    cd "$HOME/agsb"
    unzip -q xray.zip
    chmod +x xray
    rm -f xray.zip geoip.dat geosite.dat  # 清理不需要的文件
    sbcore=$("$HOME/agsb/xray" version 2>/dev/null | awk '/^Xray/{print $2}')
    echo "已安装Xray官方版本：$sbcore"
else
...
if [ ! -e "$HOME/agsb/sing-box" ]; then
# 获取最新版本号
echo "正在获取Sing-box最新版本信息..."
latest_version=$(curl -s https://api.github.com/repos/SagerNet/sing-box/releases/latest | grep '"tag_name"' | cut -d'"' -f4)
if [ -z "$latest_version" ]; then
    echo "获取版本信息失败，使用备用方法..."
    latest_version="v1.9.7"  # 备用版本
fi
echo "下载Sing-box官方版本：$latest_version"

# 根据架构设置下载文件名
case $cpu in
    amd64) sb_file="sing-box-$latest_version-linux-amd64.tar.gz";;
    arm64) sb_file="sing-box-$latest_version-linux-arm64.tar.gz";;
    *) echo "不支持的架构: $cpu" && exit 1;;
esac

# 下载并解压Sing-box
curl -Lo "$HOME/agsb/sing-box.tar.gz" -# --retry 2 "https://github.com/SagerNet/sing-box/releases/download/$latest_version/$sb_file"
if [ $? -eq 0 ] && [ -f "$HOME/agsb/sing-box.tar.gz" ]; then
    cd "$HOME/agsb"
    tar -xzf sing-box.tar.gz --strip-components=1
    chmod +x sing-box
    rm -f sing-box.tar.gz LICENSE README.md  # 清理不需要的文件
    sbcore=$("$HOME/agsb/sing-box" version 2>/dev/null | awk '/version/{print $NF}')
    echo "已安装Sing-box官方版本：$sbcore"
else
    echo "Sing-box下载失败，请检查网络连接"
    exit 1
fi
fi
...
if command -v openssl >/dev/null 2>&1; then
    ...
    if [ -f "$HOME/agsb/private.key" ] && [ -f "$HOME/agsb/cert.pem" ]; then
        echo "SSL证书生成成功"
    else
        echo "SSL证书生成失败"
        exit 1
